...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
	...
	<div class="row">
		<div class="col-md-12" id="wl-wrapper">
			...
			<table class="shop_table cart wishlist_table my-lists-table" cellspacing="0">
				<thead>
				<tr>
				...
				if( count( $wishlist ) > 0 ) :
						...
						if( $product_obj !== false && $product_obj->exists() ) : ?>
							<tr id="yith-wcwl-row-<?php echo $values['ID'] ?>">
								...
								<td class="product-thumbnail">
									<a href="javascript:void(0)" onclick="remove_item_from_wishlist( '<?php echo esc_url( $yith_wcwl->get_remove_url( $values['ID'] ) )?>', 'yith-wcwl-row-<?php echo $values['ID'] ?>');" class="remove" title="<?php _e( 'Remove this product', 'yit' ) ?>">&times;</a>
									
									<?php
									# start: modified by Arlind Nushi
									if(has_post_thumbnail($product_obj->id))
									{
										$cart_img = laborator_show_img($product_obj->id, 'shop-thumb-2'); 
									}
									else
									{
										$attachment_ids = $product_obj->get_gallery_attachment_ids();
								...
								<td class="product-name">
									...
									<?php
									# Rating			
									if ( get_option( 'woocommerce_enable_review_rating' ) != 'no' && $product_obj->get_rating_count() ):
									?>
									
									<div class="rating filled-<?php echo intval($product_obj->get_average_rating()); echo $product_obj->get_average_rating() - intval($product_obj->get_average_rating()) > .49 ? ' and-half' : ''; ?>">
								...
								<?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
									<td class="product-stock-status heading-font up">
										<?php
					...
					<tr>
						<td colspan="6" class="wishlist-empty up">
							<center><?php _e( 'No products were added to the wishlist', 'yit' ) ?></center>
						</td>
					</tr>
				<?php
				endif;
