...
<div class="images product-images">
	...
	<?php
		if ( has_post_thumbnail() || count($attachment_ids) ) {
			...
			# start: modified by Arlind Nushi
			if(has_post_thumbnail())
				$image = laborator_show_img($post->ID, 'shop-thumb-4');
			else
			...
			echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image zoom" data-lightbox-gallery="main-images" data-rel="prettyPhoto' . $gallery . '">%s %s</a>', $image_link, $image, $zoom ), $post->ID );
			...
			if ( $attachment_ids ) 
			{
				...
				foreach ( $attachment_ids as $attachment_id )
				{
					...
					echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image zoom hidden" data-lightbox-gallery="main-images" data-rel="prettyPhoto' . $gallery . '">%s %s</a>', $image_link, $image, $zoom ), $post->ID );
				}
			}
