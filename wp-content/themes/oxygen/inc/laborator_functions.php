...
function dimox_breadcrumbs($echo = true, $force_use = false)
{
	...
	if (is_home() || is_front_page()) {
		...
		if ( get_query_var('paged') ) {
			...
			$output .= ' <a class="paged">(' . __('Page', TD) . ' ' . get_query_var('paged') . ')</a>';
			#if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) $output .= ')';
		}
...
function move_array_index_after(& $array, $index, $after_index)
{
	$new_array = array();
...
function laborator_load_font_style()
{
	...
	wp_enqueue_style('primary-font', $api_url . $primary_font);
	wp_enqueue_style('heading-font', $api_url . $secondary_font);
...
.page-container .lab_wpb_banner_2 .title,
