...
<a href="admin.php?page=<?php echo $_GET['page']; ?>">&laquo; Go Back</a>
	...
	public function generate_pagination()
	{
		...
		if($max_pages > 1 && $pg_num != -1)
		{
			...
			$show_all_entries = add_query_arg($this->pagination_key, -1);
	...
	{
		?>
		<a href="<?php echo remove_query_arg("delete_{$this->instance_id}", add_query_arg(array("edit_{$this->instance_id}" => $entry_id, 'action' => 'edit'))); ?>">Edit</a> -
		<span class="trash"><a href="<?php echo remove_query_arg("edit_{$this->instance_id}", add_query_arg(array("delete_{$this->instance_id}" => $entry_id))); ?>" class="confirm_laborator_dataopt_delete">Delete</a></span>
		<?php
	}
